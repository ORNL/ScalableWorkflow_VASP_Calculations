...
def plot_data(source_path, elements_list):
    ...
    xdata_loc = [sum(data.x[:, 0] == pure_elements_dictionary[elements_list[xdata_index]]).item() / data.num_nodes for data in dataset_loc]
    ydata_loc = [sum(data.x[:, 0] == pure_elements_dictionary[elements_list[ydata_index]]).item() / data.num_nodes for data in dataset_loc]
    zdata_loc = [sum(data.x[:, 0] == pure_elements_dictionary[elements_list[zdata_index]]).item() / data.num_nodes for data in dataset_loc]
    msd_loc = [data.y[0].item() for data in dataset_loc]

    assert len(xdata_loc) == len(msd_loc)
    assert len(ydata_loc) == len(msd_loc)
    assert len(zdata_loc) == len(msd_loc)

    tuples_loc = [(xdata, ydata, zdata, msd_data) for xdata, ydata, zdata, msd_data in zip(xdata_loc, ydata_loc, zdata_loc, msd_loc)]
    ...
    if rank == 0:
        ...
        msd = [elem[3] for elem in tuples]

        q25_formation_enthalpy, q75_formation_enthalpy = np.percentile(msd, [25, 75])
        bin_width_msd = 2 * (q75_formation_enthalpy - q25_formation_enthalpy) * len(msd) ** (-1 / 3)
        bins_msd = round((max(msd) - min(msd)) / bin_width_msd)
        print("Min and Maximum of Formation Energy: ", min(msd[2:-1]), " - ",
              max(msd[2:-1]))
        # print("Freedman–Diaconis number of bins:", bins_formation_enthalpy)
        plt.figure()
        plt.hist(msd, range=[0.0, 0.05], color="blue", density=False,
                 bins=100)  # density=False would make counts
        plt.ylabel('Number of configurations')
        plt.xlabel('RMSD (angstrom)')
        print("title of the plots: ", elements_list[0] + elements_list[1] + elements_list[2] + ' - BCC phase')
        plt.title(elements_list[0] + elements_list[1] + elements_list[2] + ' - BCC phase')
        plt.draw()
        plt.tight_layout()
        plt.savefig('BCC_MSD_Histogram_' + elements_list[0] + elements_list[1] + elements_list[2])

        composition_x = list(set([item[0] for item in tuples]))

        for composition in composition_x:

            filtered_tuples = [(xdata, ydata, zdata, msd_data) for (xdata, ydata, zdata, msd_data) in tuples if xdata == composition]

            ydata = [elem[1] for elem in filtered_tuples]
            msd = [elem[3] for elem in filtered_tuples]

            # plot MSD as a function of chemical composition
            fig, ax = plt.subplots()
            hist2d_norm = getcolordensity(ydata, msd)

            plt.scatter(
                ydata, msd, s=8, c=hist2d_norm, vmin=0, vmax=1
            )
            plt.clim(0, 1)
            plt.ylim((0, 0.12))
            plt.colorbar()
            plt.xlabel(elements_list[ydata_index]+" concentration")
            plt.ylabel('RMSD (angstrom)')
            plt.title(''.join(elements_list)+' - '+str(composition*100)+' at\% '+elements_list[xdata_index])
            ax.set_xticks([0.0, 0.5, 1.0])
            plt.draw()
            plt.tight_layout()
            plt.savefig("./BCC_MSD_vs_concentration_" +str(composition)+'_concentration_of_'+elements_list[xdata_index] + ".png", dpi=400)
