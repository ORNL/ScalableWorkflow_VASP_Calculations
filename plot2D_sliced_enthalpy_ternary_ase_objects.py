...
def plot_data(source_path, elements_list):
    ...
    assert len(ydata_loc) == len(formation_enthalpy_loc)
    assert len(zdata_loc) == len(formation_enthalpy_loc)

    tuples_loc = [(xdata, ydata, zdata, enthalpy) for xdata, ydata, zdata, enthalpy in zip(xdata_loc, ydata_loc, zdata_loc, formation_enthalpy_loc)]
    ...
    if rank == 0:
        ...
        for composition in composition_x:
            ...
            filtered_tuples = [(xdata, ydata, zdata, enthalpy) for (xdata, ydata, zdata, enthalpy) in tuples if xdata == composition]

            ydata = [elem[1] for elem in filtered_tuples]
            formation_enthalpy = [elem[3] for elem in filtered_tuples]
            ...
            fig, ax = plt.subplots()
            hist2d_norm = getcolordensity(ydata, formation_enthalpy_rescaled)

            plt.scatter(
                ydata, formation_enthalpy_rescaled, s=8, c=hist2d_norm, vmin=0, vmax=1
            )
            plt.clim(0, 1)
            plt.ylim((0, 70.0))
            plt.colorbar()
            ...
            plt.draw()
            plt.tight_layout()
            plt.savefig("./BCC_enthalpy_vs_concentration_" +str(composition)+'_concentration_of_'+elements_list[xdata_index] + ".png", dpi=400)
