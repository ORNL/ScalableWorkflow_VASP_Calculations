...
def plot_data(source_path, elements_list):
        ...
        xdata_loc = [sum(data.x[:, 0] == pure_elements_dictionary[elements_list[0]]).item() / data.num_nodes for data in dataset_loc]
        ydata_loc = [sum(data.x[:, 0] == pure_elements_dictionary[elements_list[1]]).item() / data.num_nodes for data in dataset_loc]
        formation_enthalpy_loc = [data.y[0].item() for data in dataset_loc]

        assert len(xdata_loc) == len(formation_enthalpy_loc)

        tuples_loc = [(composition_x, composition_y, enthalpy) for composition_x, composition_y, enthalpy in zip(xdata_loc, ydata_loc, formation_enthalpy_loc)]
        ...
        if rank == 0:
            ...
            ydata = [elem[1] for elem in tuples]
            formation_enthalpy = [elem[2] for elem in tuples]
            ...
            plt.figure()
            plt.hist(formation_enthalpy_rescaled, range=[0.0, 70.0], color="blue", density=False,
            ...
            plt.draw()
            plt.tight_layout()
            plt.savefig('BCC_Enthalpy_Histogram_' + elements_list[0] + elements_list[1] + elements_list[2])
            ...
            p = ax.scatter3D(xdata, ydata, formation_enthalpy_rescaled, c=hist2d_norm);
            ax.set_xlim(0, 1); ax.set_ylim(0, 1); ax.set_zlim(min(formation_enthalpy_rescaled), max(formation_enthalpy_rescaled));
            ax.set_xlabel(elements_list[0]+" concentration")
            ax.set_ylabel(elements_list[1]+" concentration")
            ax.set_zlabel("Formation Energy (meV/atom)")
