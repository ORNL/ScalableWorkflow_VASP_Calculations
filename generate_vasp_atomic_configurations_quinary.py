...
def generate_VASP_randomized_ternary_configurations(source_path, destination_path, atomic_increment, symmetry_savings,
    ...
    assert 5 == len(elements_list), "This function can only be used for quinary alloys"
    ...
    for num_first_element_atoms in range(0, num_atoms + 1, atomic_increment):
        ...
        for num_second_element_atoms in range(0, num_atoms - num_first_element_atoms, atomic_increment):
            ...
            for num_third_element_atoms in range(0, num_atoms - num_first_element_atoms - num_second_element_atoms, atomic_increment):

                for num_fourth_element_atoms in range(0, num_atoms - num_first_element_atoms - num_second_element_atoms - num_third_element_atoms,
                                                     atomic_increment):
                    ...
                    for idx in range(num_first_element_atoms + num_second_element_atoms, num_first_element_atoms + num_second_element_atoms + num_third_element_atoms):
                        chemical_composition[idx] = elements_list[2]

                    for idx in range(num_first_element_atoms + num_second_element_atoms + num_third_element_atoms, num_atoms):
                        chemical_composition[idx] = elements_list[3]

                    for idx in range(num_first_element_atoms + num_second_element_atoms + num_third_element_atoms + num_fourth_element_atoms, num_atoms):
                        chemical_composition[idx] = elements_list[4]
...
if __name__ == '__main__':
    ...
    source_path = current_directory + '/template_BCC_Nb-Ta-V-Zr-Hf'
    destination_path = current_directory + '/bcc/Nb-Ta-V-Zr-Hf'
    atomic_increment = 8
    num_atomic_configurations_per_composition = 1
